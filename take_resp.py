...
while True:
    ...
    a = f'Замена на {today.strftime("%d")}.{today.strftime("%m")}.{today.year}г., {days_naming[today_day]}'
    b = f'Замена на {tomorrow.strftime("%d")}.{tomorrow.strftime("%m")}.{tomorrow.year}г., {days_naming[next_day]}'
    ...
    #######################################################################
    profile = webdriver.FirefoxProfile('/home/author/.mozilla/firefox/7jrpf3xv.default-esr')
    ...
    #####################
    try:
        driver.get('https://portal.petrocollege.ru/Lists/List/AllItems.aspx')
        time.sleep(2)
